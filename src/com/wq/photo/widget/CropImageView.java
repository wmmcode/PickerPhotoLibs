...
public class CropImageView extends View {
    ...
    protected float minZoomIn = 0.5f;// 最小缩小到多少倍

    protected Drawable mDrawable;// 原图
    protected FloatDrawable mFloatDrawable;// 浮层
    protected Rect mDrawableSrc = new Rect();// 原始矩形大小
    protected Rect mDrawableDst = new Rect();// 放大（缩小）后的矩形大小
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getPointerCount() > 1) {
            // 只用于第一次：默认初始化是STATUS_Touch_SINGLE，所以如果是多点触摸，那么开始置为STATUS_TOUCH_MULTI_START
            ...
            // 如果是开始是多点触摸，突然，又换成单点，那么就清空记录的数据，并只记录当前的点
            ...
            mStatus = STATUS_Touch_SINGLE;// 状态置为单点
        }

        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                //记录第一下的点。（如果是多点，会在event.getPointerCount() > 1 的位置，再设置多点）
                oldX = event.getX();
                oldY = event.getY();
                break;

            case MotionEvent.ACTION_UP:
                //缩小小于1.0倍的时候，这边界判断会导致抖动，暂时关闭。
//                checkBounds();
                break;
            ...
            case MotionEvent.ACTION_MOVE:
                if (mStatus == STATUS_TOUCH_MULTI_TOUCHING) {
                    ...
                    float newy_0 = event.getY(0);// 第一根手指
                    ...
                    int _newHeight = (int) ((float) _newWidth / oriRationWH);// 放大后的宽高

                    float tmpZoomRation = (float) _newWidth / (float) mDrawableSrc.width();// 重新获取缩放比例。
                    if (tmpZoomRation >= maxZoomOut) {// 大于最大倍数，就用最大倍数缩放
                    ...
                    oldx_1 = newx_1;
                    oldy_1 = newy_1;
                } else if (mStatus == STATUS_Touch_SINGLE) {// 单点
...
//                                isChange = true;
//                            }
//                            Log.e("CropImageView", "ACTION_MOVE  mDrawableDst.left = "+mDrawableDst.left+"  mDrawableDst.top ="+mDrawableDst.top+"   getWidth()="+getWidth()+"    newLeft = "+newLeft);
//
...
//                                Log.e("CropImageView", "ACTION_MOVE  newLeft = "+newLeft+"  newTop="+newTop);
//                            }
//                        }
                        invalidate();
                    }
    ...
    protected void configureBounds() {
        if (isFrist) {
             ...
             * (int)(mDrawable.getIntrinsicWidth()*scale+0.5f)); int h=(int)
             * (w/oriRationWH);
             * 
             * int left = (getWidth()-w)/2; int top = (getHeight()-h)/2; int
             * right=left+w; int bottom=top+h;
             */
    ...
    public Bitmap getCropImage() {
        try {
            ...
            // Bitmap newRet=Bitmap.createScaledBitmap(ret, cropWidth,
            // cropHeight, false);
            // ret.recycle();
            // ret=newRet;
            // Bitmap b = ImageUtil.zoomBitmap(ret, cropWidth, cropHeight,
            // false);
            // ret.recycle();
            // ret = b;
